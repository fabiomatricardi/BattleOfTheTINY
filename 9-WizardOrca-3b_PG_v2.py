...
convHistory = ''
modelfile = "model/q4_0-wizard-orca-3b.gguf"
repetitionpenalty = 1.15
contextlength=4096
logfile = 'WizardOrca3.b_logs.txt'
print("loading model...")
...
with gr.Blocks(theme='Medguy/base2') as demo:   #theme=gr.themes.Glass()  #theme='remilia/Ghostly'
    ...
    with gr.Row(variant='compact'):
            with gr.Column(scale=3):            
                gr.Image(value='./wizardorca.png', 
                        show_label = False, 
                        show_download_button = False, container = False)              
            with gr.Column(scale=10):
                gr.HTML("<center>"
                + "<h3>Prompt Engineering Playground!</h3>"
                + "<h1>🧙‍♂️🐋 Wizard-Orca-3B - 2K context window</h1></center>")  
                with gr.Row():
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            max_len = gr.Slider(label="Maximum output lenght", minimum=10,maximum=(contextlength-1500),step=2, value=900)
            gr.Markdown(
            """
            ...
            gr.Markdown(
            f"""
            - **Prompt Template**: Wizard 🧙‍♂️🐋
            - **Repetition Penalty**: {repetitionpenalty}
            - **Context Lenght**: {contextlength} tokens
            - **LLM Engine**: llama-cpp
            - **Model**: 🧙‍♂️🐋 Aryanne/Wizard-Orca-3B-gguf
            - **Log File**: {logfile}
            """) 
