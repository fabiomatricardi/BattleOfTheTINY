...
convHistory = ''
modelfile = "models/TinyMistral-248M-SFT-v4.Q8_0.gguf"
repetitionpenalty = 1.15
contextlength=1024
logfile = 'TinyMistral248M_logs.txt'
print("loading model...")
...
with gr.Blocks(theme='Medguy/base2') as demo:   #theme=gr.themes.Glass()  #theme='remilia/Ghostly'
    ...
    with gr.Row(variant='compact'):
            with gr.Column(scale=3):            
                gr.Image(value='https://github.com/fabiomatricardi/GradioStudies/raw/main/20231206/mistral248.jpg', 
                        show_label = False, height = 160,
                        show_download_button = False, container = False,)              
            with gr.Column(scale=10):
                gr.HTML("<center>"
                + "<h3>Prompt Engineering Playground!</h3>"
                + "<h1>〽️ TinyMistral-248M-SFT-v4 - 1024 context window</h2></center>")  
                with gr.Row():
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            max_len = gr.Slider(label="Maximum output lenght", minimum=10,maximum=(contextlength-150),step=2, value=250)
            gr.Markdown(
            """
            Fill the System Prompt and User Prompt
            And then click the Button below
            """)
            btn = gr.Button(value="〽️ Generate", variant='primary')
            gr.Markdown(
            f"""
            - **Prompt Template**: Mistral instruct 〽️
            - **Repetition Penalty**: {repetitionpenalty}
            - **Context Lenght**: {contextlength} tokens
            - **LLM Engine**: llama-cpp
            - **Model**: 〽️ TinyMistral-248M-SFT-v4.Q8_0.gguf
            - **Log File**: {logfile}
            """) 


        with gr.Column(scale=4):
            txt = gr.Textbox(label="System Prompt", lines=1, interactive = False, value = '', placeholder='This model does not have any system message...')
